...
int main() {
    ...
    float3 **img_gpu;
    cudaMallocManaged((void**)&img_gpu, height * sizeof(float3*));

    for(int i=0; i<height; i++) cudaMallocManaged( (void**)&img_gpu[i], width*sizeof(float3));


    iStart = cpuSecond();

    printf("Computing on GPU...\n");
    gpu_compute<<<grid, block>>>(width, height, O, Q, position, radius, L, ambient, diffuse, color, specular_c, specular_k, color_light, step, img_gpu);
    ...
    for(int i=0; i<height; i++) cudaFree(img_gpu[i]);
    cudaFree(img_gpu);
